...
namespace ASC.Web.Areas.Accounts.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        // Khai báo rõ interface IEmailSender của UI Services để tránh xung đột với IEmailSender<T> của .NET 8
        private readonly Microsoft.AspNetCore.Identity.UI.Services.IEmailSender _emailSender;
        private readonly SignInManager<IdentityUser> _signInManager;

        public AccountController(UserManager<IdentityUser> userManager, Microsoft.AspNetCore.Identity.UI.Services.IEmailSender emailSender, SignInManager<IdentityUser> signInManager)
        {
        ...
        public async Task<IActionResult> ServiceEngineers(ServiceEngineerViewModel serviceEngineer)
        {
            ...
            // Kiểm tra Registration khác null để loại bỏ warning CS8602
            if (serviceEngineer.Registration != null && serviceEngineer.Registration.IsEdit)
            {
                var user = await _userManager.FindByEmailAsync(serviceEngineer.Registration.Email);

                // Kiểm tra user khác null trước khi thực hiện logic để bỏ warning CS8604
                if (user != null)
                {
                    ...
                    // Kiểm tra claim tồn tại trước khi xóa để loại bỏ warning CS8602
                    if (isActiveClaim != null)
                    {
                        await _userManager.RemoveClaimAsync(user, new System.Security.Claims.Claim(isActiveClaim.Type, isActiveClaim.Value));
                    }
            ...
            else if (serviceEngineer.Registration != null)
            {
            ...
            if (serviceEngineer.Registration != null)
            {
